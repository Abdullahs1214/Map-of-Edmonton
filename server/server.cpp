...
//CCID: asherwan
//AnonID: 1000338172
//Course: CMPUT 275, Winter 2023
//Assignment: Assignment 1 part 2: assignment_part2
...
Point create_coords(const string lat, const string lon){
  return Point {static_cast<long long>(stod(lat) * 100000), static_cast<long long>(stod(lon) * 100000)};
}
...
int main() {
  ...
  while (true){
    ...
    sPoint = create_coords(coord[0],coord[1]); //convert to a form which findClosest is able to utilize them
    ePoint = create_coords(coord[2],coord[3]);
